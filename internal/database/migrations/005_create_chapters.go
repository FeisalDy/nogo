...
type Chapter struct {
	gorm.Model
	NovelId   uint   `json:"novel_id" gorm:"not null;index:idx_novel_chapter;uniqueIndex:idx_novel_chapter_unique"`
	Novel     *Novel `json:"novel" gorm:"foreignKey:NovelId;constraint:OnUpdate:CASCADE,OnDelete:CASCADE"`
	Number    int    `json:"number" gorm:"not null;index:idx_novel_chapter;uniqueIndex:idx_novel_chapter_unique"`
