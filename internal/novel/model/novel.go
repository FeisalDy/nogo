...
// Novel represents the novel aggregate in the Novel domain.
// Following DDD principles:
// - Only stores IDs for cross-domain references (CreatedBy, CoverMediaId)
// - Does NOT import User or Media models
// - Cross-domain data fetching handled by Application layer
type Novel struct {
	gorm.Model
	...
	// Foreign keys to other domains - stored as IDs only
	CoverMediaId *uint `json:"cover_media_id" gorm:"index"` // Reference to Media domain
	CreatedBy    *uint `json:"created_by" gorm:"index"`     // Reference to User domain
}
...
type NovelTranslation struct {
	...
	TranslatorId *uint   `json:"translator_id" gorm:"index"` // Reference to User domain
}
